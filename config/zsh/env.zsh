...
# opencode
export PATH=/home/ra4ing/.opencode/bin:$PATH
...
# fnm - fix /run/user/UID permissions for WSL
export XDG_RUNTIME_DIR="/run/user/$(id -u)"
if [ ! -d "$XDG_RUNTIME_DIR" ]; then
  mkdir -p "$XDG_RUNTIME_DIR"
  chmod 700 "$XDG_RUNTIME_DIR"
elif [ ! -O "$XDG_RUNTIME_DIR" ]; then
  # Directory exists but owned by root (common WSL issue) - reclaim it
  new_dir="$HOME/.xdg_runtime"
  mkdir -p "$new_dir"
  chmod 700 "$new_dir"
...
# XDG_RUNTIME_DIR already set above (before fnm init)
